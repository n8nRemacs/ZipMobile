...
Использование:
  # Один прайс:
  python3 parse_profi_v2.py --url "https://..." --city "Москва" --shop "Савеловский"

  # Все прайсы:
  python3 parse_profi_v2.py --all
...
def parse_excel(file_path: str, city: str, shop: str) -> list:
    """Парсинг Excel файла"""
...
def parse_single(url: str, city: str, shop: str) -> int:
...
def main():
    ...
    parser.add_argument("--shop", help="Shop name")
    parser.add_argument("--all", action="store_true", help="Parse all configured price lists")
    parser.add_argument("--no-truncate", action="store_true", help="Don't truncate table before insert")
    ...
    try:
        if args.all:
            ...
            # Если город/магазин не указаны, пытаемся получить из конфига
            info = get_info_by_url(args.url)
            city = args.city or info.get("city", "Неизвестно")
            shop = args.shop or info.get("shop", "Неизвестно")

            products = parse_single(args.url, city, shop)
            save_to_db(products, truncate=not args.no_truncate)
            result = 0
        else:
            # По умолчанию - Астрахань (для обратной совместимости)
            url = "https://www.siriust.ru/club/price/Astraxan.xls"
            products = parse_single(url, "Астрахань", "Профи")
            save_to_db(products)
            result = 0
