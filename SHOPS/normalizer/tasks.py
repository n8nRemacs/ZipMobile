"""
Background task manager — normalizer_tasks table operations
"""
import asyncio
import json
import traceback
...
async def create_task(task_type: str, params: dict | None = None) -> UUID:
    ...
    row = await pool.fetchrow(
        """INSERT INTO normalizer_tasks (task_type, status, params)
...
async def update_task(
    ...
    *,
    ...
    args.append(task_id)
    await pool.execute(
        f"UPDATE normalizer_tasks SET {', '.join(sets)} WHERE id = ${idx}",
        *args,
    )
...
async def get_task(task_id: UUID) -> TaskResponse | None:
    pool = get_pool()
    row = await pool.fetchrow("SELECT * FROM normalizer_tasks WHERE id = $1", task_id)
    if not row:
        return None
    return TaskResponse(
        task_id=row["id"],
